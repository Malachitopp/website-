...
const EMAIL = 'malachi.topp@malachitopp.com'
// An envelope (GitHub's mail octicon, on the same 16 × 16 grid)
const MAIL_MARK =
...
const SHORTCUTS = [
  { name: 'GitHub', href: GITHUB, mark: GITHUB_MARK },
  { name: 'Email', href: `mailto:${EMAIL}`, mark: MAIL_MARK, title: EMAIL },
]
...
// a shortcut for every place my work lives (GitHub and email so far). From the overview the whole
          ...
          {SHORTCUTS.map((shortcut) => (
            <li key={shortcut.href}>
              <a
                ...
                // a mail link opens the mail app, not a page, so it mustn't leave an empty tab behind
                target={shortcut.href.startsWith('mailto:') ? undefined : '_blank'}
                rel="noreferrer"
              >
